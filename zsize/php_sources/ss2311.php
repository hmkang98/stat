...
$titleStr = "검정력 계산(one-sided) : p<sub>0</sub>> p<sub>1</sub>";
?>
<html>
<head>
...
if (strlen($alpha) < 1 || strlen($n) < 1 || strlen($p0) < 1 || strlen($p1) < 1 ) {
	alertBack("alpha 와 n, p<sub>0</sub>,p<sub>1</sub>을 다시 입력하시기 바랍니다.");
	exit;
}

if ($n <= '0.0') {
	alertBack("n는 0보다 큰 값을 가집니다.");
	exit;
}
...
$za = Quantile(1-$alpha);
$beta = ($za * sqrt($p0*(1-$p0)/$n) - ($p0-$p1)) / sqrt($p1*(1-$p1)/$n);
...
<body>
	...
	<tr>
		<td bgColor="#ff0000">
			...
			<tr BGCOLOR="#f5f5f5">
				<TD> 유의수준(α) = <?=$alpha?>, 표본수 = <?=$n?>, p<sub>0</sub>= <?=$p0?>, p<sub>1</sub>= <?=$p1?>인 경우<p>
