...
if (strlen($alpha) < 1 || strlen($beta) < 1 || strlen($p0) < 1 || strlen($p1) < 1 || strlen($p2) < 1 ) {
	alertBack("alpha 와 beta, p<sub>0</sub>,p<sub>1</sub>,p<sub>2</sub>을 다시 입력하시기 바랍니다.");
	exit;
}
...
if (($p2 <='0.0') || ($p2 >= '1.0')) {
	alertBack("p<sub>2</sub>는 0부터 1사이의 값을 가집니다.");
	exit;
}
if (abs($p1-$p2) <= '0.0') {
	alertBack("p<sub>1</sub>과p<sub>2</sub>는 서로 다른 값을 가집니다.");
	exit;
}

$za = Quantile(1-$alpha/2);
$zb = Quantile($beta);
$up = pow($za*sqrt(2*$p0*(1-$p0))+$zb*(sqrt($p1*(1-$p1)+$p2*(1-$p2))),2);
$down = pow($p1-$p2,2);
$ss1 = $up/$down;
$ss11 = ceil($ss1);
...
<body>
	...
	<tr>
		<td bgColor="#ff0000">
			...
			<tr BGCOLOR="#f5f5f5">
				<TD> 유의수준(α) = <?=$alpha?> , 검정력(β) = <?=$beta?>, p<sub>0</sub>= <?=$p0?>, p<sub>1</sub>= <?=$p1?>인경우, p<sub>2</sub>= <?=$p2?>인경우<p>
