...
$titleStr = "검정력 계산(one-sided) : μ<sub>0</sub>> μ<sub>1</sub>인 경우";
?>
<html>
<head>
...
if (strlen($alpha) < 1 || strlen($n) < 1 || strlen($ovalue) < 1|| strlen($alter) < 1|| strlen($sigma2) < 1) {
	alertBack("alpha 와 표본수, μ<sub>0</sub>,μ<sub>1</sub>,분산을 다시 입력하시기 바랍니다.");
	exit;
}
...
<body>
	...
	<tr>
		<td bgColor="#ff0000">
			...
			<tr BGCOLOR="#f5f5f5">
				<TD> 유의수준(α) = <?=$alpha?>, 각 집단의 표본수 = <?=$n?>, μ<sub>0</sub>= <?=$ovalue?>, μ<sub>1</sub>= <?=$alter?>, 분산(σ²) = <?=$sigma2?>인 경우<p>
